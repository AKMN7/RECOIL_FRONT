	...
	<section class="container m-auto p-4 space-y-14">
		...
		<div class="w-full h-fit flex items-center justify-center">
			<div class="relative w-4/5">
				<input
					class="appearance-none border-2 pl-4 pr-10 border-gray-300 hover:border-gray-400 transition-colors rounded-md w-full py-3 px-3 text-gray-800 leading-tight focus:outline-none focus:ring-primary focus:border-primary focus:shadow-outline"
					id="prompt"
					type="text"
					placeholder="What's on your mind..."
					v-model="userPrompt"
					v-on:keyup.enter="generateImage" />

				<button class="absolute right-0 inset-y-0 flex items-center" @click="generateImage">
					<svg
		...
		<div class="w-full h-fit flex items-center justify-center">
			...
			<the-loader v-if="isLoading" :text="'Generating Image'" />
		</div>
	</section>
	<the-footer />
</template>
